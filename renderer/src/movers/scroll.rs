    ...
    
{
    ...
    fn get_origin(&self) -> (i32, i32) {
        let orig_x: i32 = if self.delta_x < 0 {
            // Scroll left, start from right side
            (self.target_width - 1) as i32
        } else if (self.delta_x) > 0 {
            // Scoll right, start from left side
            1 - (self.image.width() as i32)
        } else {
            // No X scroll
            0
        };
        let orig_y: i32 = if self.delta_y < 0 {
            // Scroll up, start from bottom side
            (self.target_height - 1) as i32
        } else if (self.delta_y) > 0 {
            // Scoll down, start from top side
            1 - (self.image.height() as i32)
        } else {
            // No Y scroll
            0
        };
    ...
    P::Subpixel: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let mut img = ImageBuffer::<P, Vec<P::Subpixel>>::new(self.target_width, self.target_height);
        // img.copy_from(&v, x, y).unwrap();
        blit(&self.image, &mut img, self.orig_x, self.orig_y);

        let (nox, noy) = self.get_origin();
        // Loop back on out of rage
        self.orig_x = self.orig_x + self.delta_x;
        if (self.orig_x <= -(self.image.width() as i32))
            || (self.orig_x >= (self.target_width as i32))
        {
            self.orig_x = nox;
        }
        self.orig_y = self.orig_y + self.delta_y;
